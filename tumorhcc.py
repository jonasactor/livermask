...
def module_mid(model, depth, filters=16):
    if options.fanout and depth < options.depth:
        filters = filters*2
    if depth==0:
...
def PredictModel(model=options.predictmodel, image=options.predictimage, outdir=options.segmentation):
  if (model != None and image != None and outdir != None ):
    ...
    segout_int   = (segout_float[...,0] >= options.segthreshold).astype(SEG_DTYPE)
...
###
##########################
def PredictModelFromNumpy(model=options.predictmodel, image=None, imageheader=None, outdir=options.segmentation):
...
def OneKfold(k=options.kfolds, i=0, datadict=None):
    modelloc = TrainModel(kfolds=k, idfold=i) 
    ...
    for idtest in test_set:
        ...
        segloc  = '%s/%s' % (options.rootlocation, datadict[idtest]['label'])
        outloc  = '%s/label-%04d.nii.gz' % (baseloc, idtest)
        sumscore = 0
        if options.numepochs > 0 and options.makepredictions: # doing K-fold prediction as I train

            imagepredict = nib.load(imgloc)
            imageheader  = imagepredict.header
            numpypredict = imagepredict.get_data().astype(IMG_DTYPE)
            allseg       = nib.load(segloc).get_data().astype(SEG_DTYPE)
            ...
            image_liver = image_liver.astype(IMG_DTYPE)
