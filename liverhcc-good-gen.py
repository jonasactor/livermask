...
def get_unet(_depth=options.depth, _filters=options.filters, _num_classes=1):
    if options.gpu > 1:
        with tf.device('/cpu:0'):
            ...
            layer_adj = Activation('hard_sigmoid')(layer_adj)
            layer_mid = module_mid(layer_adj, depth=_depth, filters=_filters)
            for i in range(options.nu):
...
#      fill_mode='nearest',
  ...
  train_generator = train_datagen.flow(x_train[TRAINING_SLICES,:,:,np.newaxis],
                        y_train_liver[TRAINING_SLICES,:,:,np.newaxis],
                        batch_size=options.trainingbatchliver)
  test_generator = test_datagen.flow(x_train[VALIDATION_SLICES,:,:,np.newaxis],
                        y_train_liver[VALIDATION_SLICES,:,:,np.newaxis],
                        batch_size=options.trainingbatchliver)


  history_liver = model_liver.fit_generator(
                        train_generator,
                        steps_per_epoch=x_train.shape[0] / options.trainingbatchliver,
