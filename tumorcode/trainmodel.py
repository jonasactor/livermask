...
def TrainModel(idfold=0):
  ...
  x_masked = x_train_typed * y_train_liver.astype(settings.IMG_DTYPE) - (1.0 - y_train_liver.astype(settings.IMG_DTYPE))
...
#          zoom_range=0.1,
  ...
  y_pred_seg   = (np.multiply(y_pred_float[...,0], y_train_liver[VALIDATION_SLICES]) >= settings.options.segthreshold).astype(settings.SEG_DTYPE)
  ...
  maskednii     = nib.Nifti1Image(x_masked     [VALIDATION_SLICES,:,:] , None )
