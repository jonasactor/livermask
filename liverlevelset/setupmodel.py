  ...
  with open(settings.options.dbfile, 'r') as csvfile:
    ...
    for row in myreader:
      ...
      if( nslice  == restruth.shape[2]):
        ...
        datamatrix ['image_dx']                       = np.repeat(1.0/(float(imagedata.header['pixdim'][1])*(settings._globalexpectedpixel / settings.options.trainingresample)), nslice)
        datamatrix ['image_dy']                       = np.repeat(1.0/(float(imagedata.header['pixdim'][2])*(settings._globalexpectedpixel / settings.options.trainingresample)), nslice)
